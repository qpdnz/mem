...
| `3core-decision-table.ts` | 条件×結果の判定表（ルールマトリクス）      | なし   |
...
> **`3core-policy` と `3core-decision-table` の使い分け：**
> `3core-policy` → 判定フローのオーケストレーション（前処理・後処理・理由付与）
> `3core-decision-table` → 条件×結果のマトリクス定義と照合ロジック

> **ディシジョンテーブル実装ルール（推奨）：**
> `if` を連続でつなげて直接処理しない。`[]` の判定テーブル（条件 + action）から最初に実行すべき処理を `enum` に確定し、最後は `switch` で処理呼び出しのみ行う。

```ts
// 3core-decision-table.ts
...
export const EXPENSE_DECISION_TABLE: DecisionRule[] = [
  { when: (x) => !x.isSubmittedInTime, action: ExpenseAction.Reject },
  { when: (x) => !x.isManager && x.amount > 50000, action: ExpenseAction.RequireManagerApproval },
  { when: () => true, action: ExpenseAction.AutoApprove }, // デフォルト
];
```

```ts
// 3core-policy.ts
import { EXPENSE_DECISION_TABLE, ExpenseAction } from "./3core-decision-table";

export async function applyExpensePolicy(input: {
  amount: number;
  isManager: boolean;
  isSubmittedInTime: boolean;
}) {
  const matched = EXPENSE_DECISION_TABLE.find((rule) => rule.when(input));
  const action = matched?.action ?? ExpenseAction.AutoApprove; // 先に処理方針を確定

  switch (action) {
    case ExpenseAction.Reject:
      return rejectExpense();
    case ExpenseAction.RequireManagerApproval:
      return requestManagerApproval();
    case ExpenseAction.AutoApprove:
...
features/record-expense/
  ...
  3core-decision-table.ts ← 条件から実行アクションを決定（enum返却）
  4cmd-build.ts
  4cmd-transaction.ts
  4cmd-run.ts
  5out-formatter.ts
...
6. **分岐は enum + switch に統一** — `if` 連鎖で直接処理せず、`3core-decision-table` で行き先を決めて `3core-policy` の `switch` で実行する
